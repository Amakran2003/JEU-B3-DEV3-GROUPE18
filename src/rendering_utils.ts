 ...
 */
export function configurePixelArtRendering(ctx: CanvasRenderingContext2D): void {
 ...
 */
export function calculatePixelPerfectDimensions(sourceWidth: number, targetWidth: number): number {
 ...
 */
export function drawPixelPerfect(
 ...
 * @param dh Hauteur destination sur le canvas
 */
export function drawFlippedPixelPerfect(
    ctx: CanvasRenderingContext2D,
    img: CanvasImageSource,
    sx: number, sy: number, sw: number, sh: number,
    dx: number, dy: number, dw: number, dh: number
