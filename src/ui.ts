...
function showMenu() {
    ...
    (document.getElementById('game-ui') as HTMLElement).style.display = 'none';
    forceHideSettings();
...
function showGame() {
    ...
    (document.getElementById('game-ui') as HTMLElement).style.display = 'flex';
    forceHideSettings();
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (playBtn) playBtn.onclick = showGame;
    // Accueil
    ...
    if (closeSettings) closeSettings.onclick = forceHideSettings;

    // Empêcher la fermeture du modal par clic sur le fond
    const modal = document.getElementById('settings-modal');
    if (modal) {
        modal.addEventListener('click', (e) => {
            if (e.target === modal) forceHideSettings();
        });
    }
