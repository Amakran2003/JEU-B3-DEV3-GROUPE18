...
export function drawPlayerAnimation(player: Joueur1 | Joueur2, x: number, y: number, width: number, height: number, debugColor?: string) {
    let currentAnim;
...
export function redessiner() {
  ...
  for (let x = 0; x < W; x += spikesWidth) {
    drawPixelPerfect(
      context,
      background_pics,
      0, 0, background_pics.width, background_pics.height,
      x, spikesY,
      spikesWidth, spikesHeight
    );
  }
