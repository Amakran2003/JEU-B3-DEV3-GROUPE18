...
export let winner: 'red' | 'blue' | null = null;
...
export function determineWinnerByScore(): 'red' | 'blue' | 'tie' {
...
export function showVictoryScreen(winnerColor: 'red' | 'blue' | 'tie', timeUp: boolean = false) {
    ...
    gameOver = true;
    if (winnerColor !== 'tie') {
        winner = winnerColor as 'red' | 'blue';
    }
    ...
    if (winnerColor !== 'tie') {
        createConfetti(winnerColor as 'red' | 'blue');
    } else {
...
function createConfetti(color: 'red' | 'blue') {
    const confettiCount = 200;
    const container = document.createElement('div');
    container.style.position = 'fixed';
    container.style.top = '0';
...
export async function getPlayerLivesAsync(playerType: 'player1' | 'player2'): Promise<number> {
...
export async function handlePlayerFall(player: typeof PLAYER1 | typeof PLAYER2) {
