...
// Définir le type global pour Audio
declare global {
...
export function loadSound(name: string, path: string, volume: number = 1.0): Promise<void> {
...
export async function playSound(name: string, volume?: number): Promise<void> {
    try {
        ...
        const soundToPlay = sounds[name].audio.cloneNode() as HTMLAudioElement;
...
export async function initBackgroundMusic(): Promise<void> {
    ...
    try {
        if (backgroundMusic) {
            ...
            try {
                ...
                if (backgroundMusic) {
                    try {
                        const music = backgroundMusic as HTMLAudioElement;
                        await music.play();
...
function setupUserInteractionMusicStart(): void {
...
export async function initSounds(): Promise<void> {
    try {
        ...
        const fxVolumeSlider = document.getElementById('fx-volume') as HTMLInputElement;
        if (fxVolumeSlider) {
